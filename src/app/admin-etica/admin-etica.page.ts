...
@Component({
  selector: 'app-admin-etica',
  templateUrl: './admin-etica.page.html',
  styleUrls: ['./admin-etica.page.scss'],
})
export class AdminEticaPage implements OnInit {
  ...
  cargarEt() {
    ...
    this.apiService.getEtic().subscribe(data => {
  ...
  async guardar() {
    ...
    if (etExistente) {
      ...
      this.apiService.postEtic(this.etActual).subscribe();
    } else {
  ...
  guardarItem(et: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.postEtic(et).subscribe(); // Guardamos la ética en la API
  }
