...
export class MovilPage implements OnInit {
  ...
  cargarMoviles() {
    ...
    this.apiService.obtenerMovil().subscribe(data => {
  ...
  async guardar() {
    ...
    if (movilExistente) {
      this.apiService.guardarMovil(this.movilActual).subscribe();
    } else {
  ...
  guardarItem(movil: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.guardarMovil(movil).subscribe();
  }
}
