...
@Component({
  selector: 'app-admin-ingles',
  templateUrl: './admin-ingles.page.html',
  styleUrls: ['./admin-ingles.page.scss'],
})
export class AdminInglesPage implements OnInit {
  ...
  cargarIngs() {
    ...
    this.apiService.getrIngl().subscribe(data => {
  ...
  async guardar() {
    ...
    if (ingExistente) {
      ...
      this.apiService.postIngl(this.ingActual).subscribe();
    } else {
  ...
  guardarItem(ing: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.postIngl(ing).subscribe(); // Guardamos la entrada en la API
  }
