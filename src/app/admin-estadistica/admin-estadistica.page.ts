...
@Component({
  ...
  templateUrl: './admin-estadistica.page.html',
  styleUrls: ['./admin-estadistica.page.scss'],
})
export class AdminEstadisticaPage implements OnInit {
  ...
  cargarEsts() {
    ...
    this.apiService.getEstc().subscribe(data => {
  ...
  async guardar() {
    ...
    if (estExistente) {
      ...
      this.apiService.postEstc(this.estActual).subscribe();
    } else {
  ...
  guardarItem(est: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.postEstc(est).subscribe(); // Guardamos la estadística en la API
  }
