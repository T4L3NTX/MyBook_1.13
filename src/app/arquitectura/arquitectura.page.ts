...
export class ArquitecturaPage implements OnInit {
  ...
  cargarArqs() {
    ...
    this.apiService.obtenerArqs().subscribe(data => {
  ...
  async guardar() {
    ...
    if (arqExistente) {
      ...
      this.apiService.guardarArq(this.arqActual).subscribe();
    } else {
  ...
  guardarItem(arq: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.guardarArq(arq).subscribe(); // Guardamos la arquitectura en la API
  }
}
