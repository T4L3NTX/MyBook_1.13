...
@Component({
  ...
  templateUrl: './admin-portafolio.page.html',
  styleUrls: ['./admin-portafolio.page.scss'],
})
export class AdminPortafolioPage implements OnInit {
  ...
  cargarPorts() {
    ...
    this.apiService.getPortafolio().subscribe(data => {
  ...
  async guardar() {
    ...
    if (portExistente) {
      this.apiService.postPortafolio(this.portActual).subscribe();
    } else {
  ...
  guardarItem(port: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.postPortafolio(port).subscribe();
  }
