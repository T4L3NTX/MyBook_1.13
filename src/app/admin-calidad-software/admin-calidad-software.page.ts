...
@Component({
  selector: 'app-admin-calidad-software',
  templateUrl: './admin-calidad-software.page.html',
  styleUrls: ['./admin-calidad-software.page.scss'],
})
export class AdminCalidadSoftwarePage implements OnInit {
  ...
  cargarCals() {
    ...
    this.apiService.getCalidad().subscribe(data => {
  ...
  async guardar() {
    ...
    if (calExistente) {
      ...
      this.apiService.postCalidad(this.calActual).subscribe(); 
    } else {
  ...
  guardarItem(cal: { fecha: string, fechaTexto: string, texto: string }) {
    ...
    this.apiService.postCalidad(cal).subscribe(); // Guardar en la API
  }
